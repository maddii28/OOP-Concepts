/*
  rec11.cpp
  Name - Maddhav Suneja
  Purpose: Test code for students to use for the linked list basics
  recitation.
*/
...
const Node* isSublist(const Node* pattern, const Node* target){
    ...
    while (target != nullptr) {
        ...
        while (t != nullptr && p != nullptr && t->data == p->data) {
            t = t->next;
            p = p->next;
        }

        if (p == nullptr) return target; 
        target = target->next;
    }
    return nullptr; 
};
...
const Node* sharedListUsingSet(const Node* listA, const Node* listB){
    unordered_set<const Node*> nodesSet;
    while (listA != nullptr) {
        nodesSet.insert(listA);
        listA = listA->next;
    }

    while (listB != nullptr) {
        if (nodesSet.find(listB) != nodesSet.end()) return listB;
        listB = listB->next;
    }
    return nullptr;
};
